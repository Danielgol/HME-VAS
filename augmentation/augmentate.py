...
if __name__ == '__main__':
    ...
    parser.add_argument('--n', type=int, default=2600, help='add or sub')
    ...
    if len(os.listdir("images")) > 0 and len(os.listdir("labels")) > 0:
        print("the images or labels folder is not empty!")
        sys.exit()
    ...
    while N_GENERATE > 0:
        ...
        try:
            ...
            if add_or_sub < 1:
                ...
                signal = cv2.imread("./subtraction/"+subtraction[random.randrange(0, len(subtraction)-1)], cv2.IMREAD_GRAYSCALE)
                signal = cv2.resize(signal, (int(digit_h*0.75) , int(digit_w*0.5)))
                x1 = int(psig[0] - signal.shape[1]/2)
                y1 = int(psig[1] - signal.shape[0]/2)
                x2 = int(psig[0] + signal.shape[1]/2)
                y2 = int(psig[1] + signal.shape[0]/2)
                img_synthetic[y1:y2, x1:x2] = signal & img_synthetic[y1:y2, x1:x2]
                text_label += "{} {:.6f} {:.6f} {:.6f} {:.6f}".format(11, ((x2+x1)/2)/width, ((y2+y1)/2)/height, (x2-x1)/width, (y2-y1)/height) + "\n"
            ...
            print(N_GENERATE)
            cv2.imwrite(f"./images/{N_GENERATE}.png", img_synthetic)
            open(f"./labels/{N_GENERATE}.txt", "w").close()
            with open(f"./labels/{N_GENERATE}.txt", "a+") as f:
               f.write(text_label)
               f.close()
            N_GENERATE -= 1
        except:
